...
class _InventoryLedgerScreenState extends ConsumerState<InventoryLedgerScreen> {
  ...
  Widget build(BuildContext context) {
    final ledgerAsync = ref.watch(inventoryLedgerProvider(_query));

    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: const ModuleAppBar(title: 'Inventory Ledger'),
      body: Column(
        children: [
          _buildToolbar(),
          Expanded(
            child: ledgerAsync.when(
              ...
              error: (e, _) => AppErrorWidget(
                message: e.toString(),
                onRetry: () => ref.invalidate(inventoryLedgerProvider(_query)),
              ),
              data: (page) {
